...
other_color(color::Int) = ifelse(color == WHITE, BLACK, WHITE)
...
xor_randn!(rand::XorRNG, n::Int) = Int(xor_randn!(rand, UInt32(n)))
...
mutable struct Board
    ...
    function Board(n::Int)
        seed = convert(UInt32, 2463534242)
        board = new()
        board.size = n
        board.komi = 0.0
...
# upper left corner.
ij2pos(board::Board, i::Int, j::Int) = (j - 1) * board.size + i
pos2ij(board::Board, pos::Int) = (1 + mod((pos - 1), board.size), 1 + fld(pos - 1, board.size))
...
on_board(board::Board, i::Int, j::Int) = i >= 1 && i <= board.size && j >= 1 && j <= board.size

function legal_move(board::Board, i::Int, j::Int, color::Int)
    is_pass_move(i, j) && return true # pass is always legal.
    board[i, j] != EMPTY && return false # already occupied
...
function has_additional_liberty(board::Board, i::Int, j::Int, libi::Int, libj::Int)
    start = ij2pos(board, i, j)
    pos = start
    while true
        (ai, aj) = pos2ij(board, pos)
        for k = 1:4
...
function provides_liberty(board::Board, ai::Int, aj::Int, i::Int, j::Int, color::Int)
  !(on_board(board, ai, aj)) && return false  # A vertex off the board does not provide a liberty.
  board[ai, aj] == EMPTY && return true # an empty vertex IS a liberty.
...
function play_move!(board::Board, i::Int, j::Int, color::Int)
    ...
    is_pass_move(i, j) && return nothing

    # If the move is a suicide we only need to remove the adjacent friendly stones.
    ...
    for k = 1:4
        ...
        if on_board(board, ai, aj) && board[pos2] == color && !same_string(board, pos, pos2)
            # The strings are linked together simply by swapping the next_stone pointers.
...
function perf_go_game(num_games_per_point::Int)
