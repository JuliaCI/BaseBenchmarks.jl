...
function perf_laplace3d(nx=290, ny=290, nz=290; verify=false)
    u1 = Array{Float32,3}(uninitialized, nx, ny, nz)
    u3 = Array{Float32,3}(uninitialized, nx, ny, nz)
    @nloops 3 k u1 begin
        if @nany 3 d->(k_d == 1 || k_d == size(u1, d))
    ...
    if verify
        u1_orig = Array{Float32,3}(uninitialized, nx, ny, nz)
        u3_orig = Array{Float32,3}(uninitialized, nx, ny, nz)
